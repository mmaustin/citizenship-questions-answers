...
/*export const QuizEight = () => {
    ...
    const displayEight = useSelector(state => state.displayQuestionsEight)
    ...
    const displayArray = displayEight.map((q, idx) => (
        q.value === 'Correct'
    ...
    const answers = quizAnswersEight.map((answer, idx) => (
...
export const QuizEight = () => {
    ...
    const [checkedState, setCheckedState] = useState(
        new Array(quizAnswersEight.length).fill(false)
    )
    ...
    const getQuestionAnswers = () => {
        ...
        for(let index of arrayOfIndices){
            answerHolder.push(quizAnswersEight[index])
        }
        return answerHolder
    }
    let answersHolder = getQuestionAnswers()

    const checkboxAnswers = quizAnswersEight.map((answer, index) => {
        return(
