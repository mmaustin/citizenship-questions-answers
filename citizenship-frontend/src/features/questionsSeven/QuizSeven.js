...
/*export const QuizSeven = () => {
    ...
    const displaySeven = useSelector(state => state.displayQuestionsSeven)
    ...
    const displayArray = displaySeven.map((q, idx) => (
        q.value === 'Correct'
    ...
    const answers = quizAnswersSeven.map((answer, idx) => (
...
export const QuizSeven = () => {
    ...
    const [checkedState, setCheckedState] = useState(
        new Array(quizAnswersSeven.length).fill(false)
    )
    ...
    const getQuestionAnswers = () => {
        ...
        for(let index of arrayOfIndices){
            answerHolder.push(quizAnswersSeven[index])
        }
        return answerHolder
    }
    let answersHolder = getQuestionAnswers()

    const checkboxAnswers = quizAnswersSeven.map((answer, index) => {
        return(
