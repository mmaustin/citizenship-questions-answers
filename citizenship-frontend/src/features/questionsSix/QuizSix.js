...
/*export const QuizSix = () => {
    ...
    const displaySix = useSelector(state => state.displayQuestionsSix)
    ...
    const displayArray = displaySix.map((q, idx) => (
        q.value === 'Correct'
    ...
    const answers = quizAnswersSix.map((answer, idx) => (
...
export const QuizSix = () => {
    ...
    const [checkedState, setCheckedState] = useState(
        new Array(quizAnswersSix.length).fill(false)
    )
    ...
    const getQuestionAnswers = () => {
        ...
        for(let index of arrayOfIndices){
            answerHolder.push(quizAnswersSix[index])
        }
        return answerHolder
    }
    let answersHolder = getQuestionAnswers()

    const checkboxAnswers = quizAnswersSix.map((answer, index) => {
        return(
