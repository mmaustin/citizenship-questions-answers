...
/*export const QuizNine = () => {
    ...
    const displayNine = useSelector(state => state.displayQuestionsNine)
    ...
    const displayArray = displayNine.map((q, idx) => (
        q.value === 'Correct'
    ...
    const answers = quizAnswersNine.map((answer, idx) => (
...
export const QuizNine = () => {
    ...
    const [checkedState, setCheckedState] = useState(
        new Array(quizAnswersNine.length).fill(false)
    )
    ...
    const getQuestionAnswers = () => {
        ...
        for(let index of arrayOfIndices){
            answerHolder.push(quizAnswersNine[index])
        }
        return answerHolder
    }
    let answersHolder = getQuestionAnswers()

    const checkboxAnswers = quizAnswersNine.map((answer, index) => {
        return(
